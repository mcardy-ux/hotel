            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="top-right-button-container">
                        <div class="btn-group">
                            <a href="{{url('regimens/create')}}" >
                                    <button type="button" class="btn btn-primary mb-1">Agregar un nuevo regimen</button>
                            </a>
                        </div>
                    </div>
                            ...
                            <div class="card-body" >
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12 col-lg-6 mb-4">
                                        <h5 class="card-title">Regimenes</h5>
                            
                                    <div class="scroll dashboard-list-with-user ps ps--active-y">
                                    
                                        @foreach($regimens as $data)
                                            <div class="d-flex flex-row mb-3 pb-3 border-bottom">
                                                <div class="pl-3">
                                                    <a href="#" onclick="verComponentes({{$data->id}})">
                                                        <p class="font-weight-medium mb-0">{{$data->codigo}} - {{$data->descripcion}}</p><br>
                                                        <p class="text-muted mb-0 text-small"><strong>Creado en:</strong> {{$data->created_at}}</p>
                                                    </a>
                                                </div>
                                                <div class="pl-3">
                                                    ...
                                                    <div class="dropdown d-inline-block">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButtonBilling" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 37px, 0px);">
                                                            <a class="dropdown-item" href="{{url('regimens', [$data->encode_id,'edit'])}}">Editar</a>
                                                            <a class="dropdown-item" onclick="show(this)" id="{{$data->encode_id}}">Eliminar</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="card dashboard-link-list">
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex flex-row">
                                                    <div class="w-50">
                                                        <ul class="list-unstyled mb-0" id="lista_componentes" name="lista_componentes">
                                                                {{-- Inicio de codigo de seleccion  --}}
                                                                <div class="d-flex flex-row align-items-center mb-5">
                                                                        <i class="glyph-icon iconsminds-cursor-select large-icon initial-height"></i>
                                                                    <div class="pl-3 pt-2 pr-2 pb-2">
                                                                            <p class="list-item-heading mb-1">Seleccione un regimen</p>
                                                                    </div>
                                                                </div>
...
@endsection
@push('scripts')
<script src="{{ asset('js/parameters/regimens/index.js') }}"></script>
<script>
    function verComponentes(id){
        let lista=document.getElementById("lista_componentes");
        let concat="";
        // RemoveOptions("ciiu");
        $.ajax({
            url: $('#index_dpto #_url').val()+"/ajax/request/components_regimen/"+id,
            ...
            if(json.success){
                for (let index = 0; index < json.data.length; index++) {
                    concat=concat+' <li class="mb-1"><span class="log-indicator border-theme-2 align-middle"></span>  '+json.data[index].value+' - '+json.data[index].secvalue+'</li>';
                }
