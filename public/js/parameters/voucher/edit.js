...
$(document).ready(function(){
    ...
    $('#edit_voucher').submit(function(event){
        ...
        let confirmacion=confirm("¿Esta seguro de editar esta información?");
          if(confirmacion){
            ...
            var data = $('#edit_voucher').serialize();
            $.ajax({
              url: $('#edit_voucher #_url').val(),
              headers: {'X-CSRF-TOKEN': $('#edit_voucher #_token').val()},
              type: 'PUT',
              cache: false,
              data: data,
              success: function (response) {
                ...
                if(json.success){
                  alert('Voucher editado exitosamente');
